    ...
    stages {
            ...
            steps {
                script {
                        withDockerRegistry(credentialsId: 'docker-cred', toolName: 'docker') {
                            sh "docker build -t lakshithaiam/emailservice:${env.DOCKER_TAG} ."
                    }
                }
            }
        }
        ...
        stage('Push Docker Image') {
            steps {
                script {
                    withDockerRegistry(credentialsId: 'docker-cred', toolName: 'docker') {
                        sh "docker push lakshithaiam/emailservice:${env.DOCKER_TAG}"
                    }
                }
            }
        }
            ...
            steps {
                script {
                    ...
                    dir('gitops-microservices-CD') {
                        sh "sed -i 's|image: lakshithaiam/emailservice:.*|image: lakshithaiam/emailservice:${env.DOCKER_TAG}|g' Kubernetes_Manifest/k8-manifest.yml"
